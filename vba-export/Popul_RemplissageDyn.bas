Attribute VB_Name = "Popul_RemplissageDyn"
' Cellule Office
Option Explicit
Option Compare Text
Function Remplissage_PopulationDyn(onglet As String)
    Dim r As Range
        ...
        '                   ThisWorkbook.Sheets("DATA").Range(SELECTION, SELECTION.offset(lastRow - 1, 0)).SpecialCells(xlCellTypeVisible).Copy
                    ...
                    If Not trouve Is Nothing Then
                        ...
                        vT = CopyColumn(rangeToCopy)
                         ...
                         colPedal = ThisWorkbook.sheets(onglet).Range("BH6:GG6").Cells.Find(What:=critere(k), LookAt:=xlWhole).Column
                ...
                Else
                        ...
                        If Not trouve Is Nothing Then
                            ...
                            colPedal = ThisWorkbook.sheets(onglet).Range("BH6:GG6").Cells.Find(What:=ThisWorkbook.sheets("structure").Cells(r.row, 4).Value, LookAt:=xlWhole).Column
                            vT = CopyColumn(ThisWorkbook.sheets("DATA").Range(nomcol(tabV(0), tabV(1)) & "2:" & nomcol(tabV(0), tabV(1)) & NEVENTS))
                            ThisWorkbook.sheets(onglet).Range(ThisWorkbook.sheets(onglet).Cells(7, colPedal), ThisWorkbook.sheets(onglet).Cells(UBound(vT) + 7, colPedal)) = Application.Transpose(vT)

'                            ThisWorkbook.Sheets("DATA").Range(nomcol(tabV(0), tabV(1)) & "2:" & nomcol(tabV(0), tabV(1)) & NEVENTS).SpecialCells(xlCellTypeVisible).Copy Destination:=.Cells(7, colPedal)
                           
'                             cel = ThisWorkbook.Sheets(onglet).Cells(7, ThisWorkbook.Sheets(onglet).Rows(6).Cells.Find(What:=ThisWorkbook.Sheets("structure").Cells(r.Row, 4).Value, LookAt:=xlWhole).Column).Address(RowAbsolute:=False, ColumnAbsolute:=False)
'                             ThisWorkbook.Sheets(onglet).Range(cel).Delete Shift:=xlUp
                        End If
       ...
       ThisWorkbook.sheets("DATA").Range("A2:A" & NEVENTS).SpecialCells(xlCellTypeVisible).Copy Destination:=.Cells(7, ThisWorkbook.sheets(onglet).Range("BH6:GG6").Cells.Find(What:="Id BdD", LookAt:=xlWhole).Column)
         ...
         If getLever(onglet) = True Then
                For i = 1 To UBound(LeverCount)
                     ...
                     While Len(ThisWorkbook.Worksheets("CONFIGURATIONS").Range("A" & j)) > 0
                          ...
                          .Columns(ThisWorkbook.sheets(onglet).Range("BH6:GG6").Cells.Find(What:=LeverCount(i), LookAt:=xlWhole).Column).replace _
...
Function CopyColumn(plageColumn As Range) As Variant
    ...
    For Each r In plageColumn.SpecialCells(xlCellTypeVisible)
            If r.EntireColumn.Hidden = False Then
                If cols = "" Then cols = fixTheDecimalSeparator(r.Value) Else cols = cols & "#" & fixTheDecimalSeparator(r.Value)
            End If
    Next r
